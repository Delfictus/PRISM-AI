...
fn find_phase_coherent_color(
    ...
    if color_scores.is_empty() {
        return Err(PRCTError::ColoringFailed(format!("Vertex {} has no available colors (need > {} colors)", vertex, max_colors)));
    }
...
mod tests {
    ...
    fn test_order_parameter_random() {
        let phases: Vec<f64> = (0..100).map(|i| (i as f64 * 0.137) % (2.0 * core::f64::consts::PI)).collect();
        let order = PhysicsCouplingService::compute_order_parameter(&phases);
    ...
    fn test_kuramoto_evolution() {
        ...
        // Evolve
        for _ in 0..100 {
            service.kuramoto_step(&mut phases, &frequencies, 0.1).unwrap();
        }
