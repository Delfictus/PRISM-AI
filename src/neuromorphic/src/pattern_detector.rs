...
impl PatternDetector {
    ...
    pub fn get_statistics(&self) -> PatternStatistics {
        self.pattern_statistics.lock().unwrap().clone()
    }

    /// Get current adaptive threshold
    pub fn get_threshold(&self) -> f64 {
        *self.adaptive_threshold.lock().unwrap()
    }

    /// Set detection threshold manually
    pub fn set_threshold(&self, threshold: f64) {
        let mut adaptive_threshold = self.adaptive_threshold.lock().unwrap();
    ...
    pub fn reset(&self) {
        ...
        *self.pattern_statistics.lock().unwrap() = PatternStatistics::default();
    }
}
