...
impl GpuProfiler {
    ...
    pub fn new(device: Arc<CudaContext>, enable_detailed: bool) -> Self {
        Self {
            device,
            metrics: DashMap::new(),
        ...
        F: FnOnce() -> Result<R>,
    {
        ...
        // Record GPU events for accurate timing
        let start_event = self.device.new_event(None)?;
        let end_event = self.device.new_event(None)?;
        ...
        let memory_transfer_time = total_time - gpu_time as f64;
        ...
        metric.gpu_compute_time_us += gpu_time as f64;
        metric.memory_transfer_time_us += memory_transfer_time.max(0.0);
        metric.call_count += 1;
    ...
    pub fn analyze_hotspots(&self) -> HotspotAnalysis {
        ...
        hotspots.sort_by(|a, b| b.time_percentage.partial_cmp(&a.time_percentage).unwrap());
